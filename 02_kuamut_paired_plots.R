...
#                   This reproduces Kuamut_paired_plots_2025.* exactly.
#  MODE = "gee"   : read the candidate table exported by
#                   01_gee_kuamut_paired_plots.js (which adds the AlphaEarth
...
PAIR_MAX       <- 300    # m
MIN_SEP        <- 100    # m   min distance to any other plot centre
ROAD_MIN       <- 25     # m   keep the 15 m plot radius clear of the road
...
ROUNDS         <- 8      # refinement passes after the greedy pass
PLOT_ID_FIELD  <- "Pl"
EPSG           <- 32650

W_CONTRAST <- 0.50; W_ACCESS <- 0.20; W_HOMOG <- 0.15; W_PROX <- 0.15
...
greedy_assign <- function(cand, min_sep = MIN_SEP, nn_margin = NN_MARGIN,
  ...
  for (nm in names(by_ref)) {
    ...
    best <- pick_best(by_ref[[nm]], taken)
    if (is.null(best)) { message("no candidate left for reference plot ", nm); next }
    picked[[nm]] <- best
  }

  # the greedy order is arbitrary: re-pick each plot against the others until
  # nothing improves
  for (it in seq_len(rounds)) {
    ...
    for (nm in worst_first) {
      ...
      best <- pick_best(by_ref[[nm]], taken)
      if (!is.null(best) && best$score > picked[[nm]]$score + 1e-9) {
        picked[[nm]] <- best; improved <- improved + 1
      }
    }
...
write_outputs <- function(new_pts, plots, stem) {
  ...
  st_write(st_buffer(g, 15), file.path(DIR_OUT, paste0(stem, ".gpkg")),
           layer = "new_plot_footprints_15m", append = TRUE, quiet = TRUE)
...
if (MODE == "local") {
  ...
  plots$ACD_ref  <- terra::extract(acd, pxy)[, 1]
  ...
  for (i in seq_len(nrow(plots))) {
    ...
    xy <- cbind(cx, cy)
    v  <- terra::extract(c(acd, acd_sd, road_d), xy)
    ...
    ref <- plots$ACD_ref[i]
    dacd <- if (plots$target[i] == "low") ref - v$acd[ok] else v$acd[ok] - ref
  ...
  print(new[, c("Pl_new", "target", "cls_ref", "cls_new", "ACD_ref", "ACD_new",
                "dACD", "d_pair", "d_other", "road_new")], digits = 4)
  cat(sprintf("\n|dACD| median %.1f, min %.1f Mg C/ha\n",
              median(new$dACD), min(new$dACD)))
  write_outputs(new, plots, "Kuamut_paired_plots_2025")
}
...
if (MODE == "gee") {
  ...
  cat(sprintf("\nranks used: %s\n",
              paste(names(table(new$rank)), table(new$rank),
                    sep = "x", collapse = " ")))
  write_outputs(new, plots, "Kuamut_paired_plots_2025_embeddings")
}
...
cat(sprintf("plots total: %d | closest pair overall: %.1f m\n",
            nrow(all_xy), min(dm)))
...
#
#  Append this to 02_kuamut_paired_plots.R, after the final sanity checks.
...
#
#  Outputs in DIR_OUT:
#     Kuamut_paired_plots_2025_map.png          overview map
#     Kuamut_paired_plots_2025_diagnostics.png  8-panel figure
#     Kuamut_paired_plots_2025_pairs_NN.png     per-pair zoom panels
#     Kuamut_paired_plots_2025_summary.csv      the numbers behind the figures
# =============================================================================
if (MODE == "local") {
  
  STEM    <- "Kuamut_paired_plots_2025"
